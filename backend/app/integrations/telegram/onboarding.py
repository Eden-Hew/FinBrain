...
def begin_onboarding(
    ...
    user_token = protect_scalar(
        db, entity_type="TGUSER", value=str(user_id), source_record_id=source_id,
        tenant_id=tenant_id,
    )
    chat_token = protect_scalar(
        db, entity_type="TGCHAT", value=str(chat_id), source_record_id=source_id,
        tenant_id=tenant_id,
    )
    row = db.scalar(select(TelegramOnboardingSession).where(
...
def submit_name(db: Session, session_id: int, name: str) -> TelegramOnboardingSession:
    ...
    row.name_token = protect_scalar(
        db, entity_type="PERSON", value=cleaned, source_record_id=source_id,
...
def submit_gmail(db: Session, session_id: int, gmail: str) -> TelegramOnboardingSession:
    ...
    row.email_token = protect_scalar(
        db, entity_type="EMAIL", value=cleaned, source_record_id=source_id,
        tenant_id=row.tenant_id,
    )
...
def submit_phone(db: Session, session_id: int, phone: str) -> TelegramOnboardingSession:
    ...
    row.phone_token = protect_scalar(
        db, entity_type="PHONE", value=cleaned, source_record_id=source_id,
        tenant_id=row.tenant_id,
    )
    ...
    result = protect_canonical_record(db, CanonicalIngestionRecord(
